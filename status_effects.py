def rage_apply(unit, value=10):
    unit.attack += value
    unit.defense -= 5
    print(f"[Эффект] {unit.name} впадает в бешенство! атака выросла на {value}, но броня упала на 5.")
...
class StatusEffect:
    ...
    def on_apply(self, unit):
        if self.custom_apply:
            self.custom_apply(unit, self.effect_value) if self.effect_value else self.custom_apply(unit)

    def on_tick(self, unit):
        if self.custom_tick:
            self.custom_tick(unit, self.effect_value) if self.effect_value else self.custom_tick(unit)
        self.duration -= 1

    def on_remove(self, unit):
        if self.custom_remove:
            self.custom_remove(unit, self.effect_value) if self.effect_value else self.custom_remove(unit)



# def rage_apply(unit, effect_value):
#     unit.attack += effect_value
#     unit.defense -= effect_value // 2
#     print(f"[Эффект] {unit.name} впадает в бешенство! атака выросла на 10, но броня упала на 5.")
#
# def rage_remove(unit, effect_value):
#     unit.attack -= effect_value
#     unit.defense += effect_value // 2
#     print(f"[Эффект] ярость у {unit.name} утихла. характеристики вернулись в норму.")
#
# def strength_apply(unit, effect_value):
#     unit.attack += effect_value
#     print(f"[Эффект] {unit.name} чувствует прилив сил! (+5 к атаке)")
#
# def strength_remove(unit, effect_value):
#     unit.attack -= effect_value
#     print(f"[Эффект] действие зелья силы на {unit.name} закончилось.")
#
# def heal_tick(unit, effect_value):
#     unit.health += effect_value
#     unit.health = min(unit.max_health, unit.health)
#     print(f"[Эффект] {unit.name} получает {effect_value} HP")
#     print(f"текущее хп {unit.name} = {unit.health}")
#
# def poison_tick(unit, effect_value):
#     unit.health -= effect_value
#     print(f"[Эффект] {unit.name} теряет 5 HP от яда.")
#
# def shield_apply(unit, effect_value):
#     unit.defense += effect_value
#     print(f"[Эффект] {unit.name} увеличивает броню на 10")
#
# def shield_remove(unit, effect_value):
#     unit.defense -= effect_value
#     print(f"[Эффект] {unit.name} действие щита закончилось")
#
# EFFECTS = {
...
#     def on_apply(self, unit):
#         if self.custom_apply:
#             self.custom_apply(unit, self.effect_value)
#
#     def on_tick(self, unit):
#         if self.custom_tick:
#             self.custom_tick(unit, self.effect_value)
#         self.duration -= 1
#
#     def on_remove(self, unit):
#         if self.custom_remove:
#             self.custom_remove(unit, self.effect_value)