...
IMAGES = {
    ...
    "bg_inventory": load_image(
        "images/bg_inventory.jpg", (WIDTH, HEIGHT), (30, 50, 40)
    ),
...
def get_enemy_image(enemy_name):
    mapping = {
        "Гоблин": "enemy_goblin",
        "Скелет": "enemy_skeleton",
        "Орк": "enemy_orc",
        "Темный Искатель": "enemy_dark_seeker",
        "Каменный Голем": "enemy_golem",
        "Вампир": "enemy_vampire",
...
def get_item_image(item):
    ...
    if "меч" in name or "экскалибур" in name or "скорбь" in name:
        return "item_sword"
    if "брон" in name or "нагрудник" in name or "кираса" in name or "поножи" in name:
        return "item_armor"
    if "здоровь" in name or "реген" in name or "фласка" in name:
        return "item_potion_heal"
    if "зелье" in name:
        return "item_potion_buff"
    if "мешок" in name or "золот" in name:
        return "item_gold"
    return "item_default"
...
class FloatingText:
    ...
    def draw(self, surface):
        if self.timer > 0:
            surface.blit(
                FONT_LARGE.render(self.text, True, self.color), (self.x, self.y)
            )
...
class PygameRPG:
    ...
    def spawn_enemy(self):
        ...
        if enemy:
            self.enemy_sprite = VisualSprite(
                600, 350, get_enemy_image(enemy.name), True
            )
    ...
    def player_attack(self):
        ...
        if (
            not enemy
            or self.player_sprite.anim_timer > 0
            or self.enemy_sprite.anim_timer > 0
        ):
            return
        ...
        if not enemy.is_alive():
            if isinstance(self.player, Warrior) and hasattr(
    ...
    def draw_health_bar(self, x, y, hp, max_hp, name):
        ...
        screen.blit(
            FONT_MED.render(f"{name} {int(hp)}/{max_hp}", True, (255, 255, 255)),
            (x, y - 25),
        )
    ...
    def run(self):
        ...
        while running:
            ...
            if self.state == "CHAR_SELECT":
                ...
                screen.blit(
                    FONT_LARGE.render("ВЫБЕРИТЕ КЛАСС", True, (255, 255, 255)),
                    (300, 100),
                )
                ...
                for btn, img, txt, cls, key in [
                    ...
                    pygame.draw.rect(
                        screen,
                        (60, 60, 80) if btn.collidepoint(mouse_pos) else (40, 40, 60),
                        btn,
                        border_radius=10,
                    )
                    screen.blit(IMAGES[img], (btn.x + 15, btn.y + 10))
                    screen.blit(
                        FONT_MED.render(txt, True, (255, 255, 255)),
                        (btn.x + 40, btn.y + 170),
                    )
                ...
                screen.blit(
                    FONT_LARGE.render(
                        "Впереди виднеется лавка торговца.", True, (255, 255, 255)
                    ),
                    (180, 200),
                )
                screen.blit(
                    FONT_LARGE.render("Зайти внутрь?", True, (255, 215, 0)), (320, 250)
                )
                ...
                pygame.draw.rect(
                    screen,
                    (50, 150, 50) if btn_yes.collidepoint(mouse_pos) else (30, 100, 30),
                    btn_yes,
                    border_radius=10,
                )
                screen.blit(
                    FONT_MED.render("Да", True, (255, 255, 255)),
                    (btn_yes.x + 45, btn_yes.y + 12),
                )
                pygame.draw.rect(
                    screen,
                    (150, 50, 50) if btn_no.collidepoint(mouse_pos) else (100, 30, 30),
                    btn_no,
                    border_radius=10,
                )
                screen.blit(
                    FONT_MED.render("Нет", True, (255, 255, 255)),
                    (btn_no.x + 40, btn_no.y + 12),
                )
                ...
                if enemy:
                    ...
                    self.draw_health_bar(
                        600, 50, enemy.health, enemy.max_health, enemy.name
                    )
                self.draw_health_bar(
                    50, 50, self.player.health, self.player.max_health, self.player.name
                )
                ...
                for btn, txt, clr, hover in [
                    ...
                    pygame.draw.rect(
                        screen,
                        hover if btn.collidepoint(mouse_pos) else clr,
                        btn,
                        border_radius=10,
                    )
                    screen.blit(
                ...
                screen.blit(
                    IMAGES["bg_shop" if self.state == "SHOP" else "bg_inventory"],
                    (0, 0),
                )
                ...
                screen.blit(
                    FONT_MED.render(
                        f"Золото: {self.player.gold}", True, (255, 255, 255)
                    ),
                    (50, 70),
                )
                ...
                if self.state == "SHOP":
                    ...
                    pygame.draw.rect(
                        screen,
                        (50, 150, 50) if self.shop_mode == "BUY" else (40, 40, 60),
                        btn_mode_buy,
                        border_radius=5,
                    )
                    pygame.draw.rect(
                        screen,
                        (50, 150, 50) if self.shop_mode == "SELL" else (40, 40, 60),
                        btn_mode_sell,
                        border_radius=5,
                    )
                    screen.blit(
                        FONT_SMALL.render("Купить", True, (255, 255, 255)),
                        (btn_mode_buy.x + 25, btn_mode_buy.y + 8),
                    )
                    screen.blit(
                        FONT_SMALL.render("Продать", True, (255, 255, 255)),
                        (btn_mode_sell.x + 20, btn_mode_sell.y + 8),
                    )

                if self.state == "SHOP":
                    items_list = (
                        self.shop.assortment
                ...
                if hovered_item:
                    ...
                    if tooltip.right > WIDTH:
                        tooltip.x = WIDTH - tooltip.width - 5
                    if tooltip.bottom > HEIGHT:
                        tooltip.y = HEIGHT - tooltip.height - 5

                    pygame.draw.rect(screen, (20, 20, 30), tooltip, border_radius=5)
                    pygame.draw.rect(
                        screen, (200, 200, 200), tooltip, 2, border_radius=5
                    )

                    price_text = (
                        f"{hovered_item.price}g"
                        if (self.state != "SHOP" or self.shop_mode == "BUY")
                        else f"Продажа: {hovered_item.price // 2}g"
                    )
                    screen.blit(
                        FONT_MED.render(
                            f"{hovered_item.name} ({price_text})", True, (255, 215, 0)
                        ),
                        (tooltip.x + 10, tooltip.y + 5),
                    )
                    screen.blit(
                        FONT_SMALL.render(
                            hovered_item.description, True, (200, 200, 200)
                        ),
                    ...
                    if self.state == "SHOP":
                        action_text = (
                            "Клик - купить"
                            if self.shop_mode == "BUY"
                            else "Клик - продать за полцены"
                        )
                    else:
                        action_text = "Клик - экипировать/использовать"
                    screen.blit(
                        FONT_SMALL.render(action_text, True, (100, 255, 100)),
                        (tooltip.x + 10, tooltip.y + 55),
                    )

                btn_close = pygame.Rect(700, 50, 150, 50)
                pygame.draw.rect(screen, (200, 50, 50), btn_close, border_radius=10)
                screen.blit(
                    FONT_MED.render("Закрыть", True, (255, 255, 255)), (730, 62)
                )
                ...
                for event in events:
                    if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                        if btn_close.collidepoint(mouse_pos):
                            ...
                            self.state == "SHOP"
                            and btn_mode_buy
                            and btn_mode_buy.collidepoint(mouse_pos)
                        ):
                            self.shop_mode = "BUY"
                        elif (
                            self.state == "SHOP"
                            and btn_mode_sell
                            and btn_mode_sell.collidepoint(mouse_pos)
                        ):
                            self.shop_mode = "SELL"
                ...
                screen.fill((50, 0, 0))
                screen.blit(
                    FONT_LARGE.render("вы мертвы...", True, (255, 255, 255)),
                ...
                screen.blit(
                    FONT_LARGE.render("вам удалось сбежать!", True, (255, 255, 255)),
                ...
                screen.blit(
                    FONT_LARGE.render(
                        "победа!! вы прошли подземелье!!", True, (255, 215, 0)
                    ),
