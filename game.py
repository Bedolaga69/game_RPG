    ...
    def _start_battle_loop(self):
        ...
        for enemy in self.enemies:
            while self.current_player.is_alive() and enemy.is_alive():
                ...
                is_player_acted = self._process_player_action(enemy)  # Вызываем ход игрока
            ...
            while True:
                ...
                if answer == "1":
                    ...
                    if answer_shop == "1":
                        ...
                        item_answer = input("введите порядковый номер предмета (начиная с 1): ")
                        if item_answer.isdigit():
                            item_answer = int(item_answer)
                            if 1 <= item_answer <= len(self.current_player.inventory):
                                item_to_sell = self.current_player.inventory[item_answer - 1]  # Теперь тоже с 1!
